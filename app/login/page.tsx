    ...
    >
      ...
      >
        <div className="login-disc-scroll min-h-0 flex-1 overflow-x-hidden overflow-y-auto overscroll-contain px-[clamp(1.65rem,11vw,2.75rem)] pt-4 sm:pt-5 pb-10 sm:pb-12 text-zinc-100">
          ...
          <p className="text-xs text-zinc-400 mb-4 text-center leading-snug">Sign in to continue</p>
          ...
          <form onSubmit={onSubmit} className="space-y-3">
            {info ? (
              <div
                role="status"
                className="text-xs text-[#8fd9c8] bg-white/5 rounded-full px-3 py-2 text-center leading-snug border border-white/10"
              >
                {info}
              </div>
            ) : null}
          ...
          <div className="mt-4 text-center space-y-2">
            {cfg?.allowRegister ? (
              <Link
                href="/create-account"
                className="block text-[#8fd9c8] hover:text-[#a8e8d4] hover:underline text-xs leading-tight px-1"
              >
            ...
            <Link
              href="/"
              className="block text-zinc-500 hover:text-zinc-300 text-xs"
            >
              ← Home
            </Link>
          </div>
...
function LoginForm() {
  ...
  useEffect(() => {
    ...
    if (qErr) {
      const labels: Record<string, string> = {
        ...
        missing_verification_token: "Invalid confirmation link.",
        invalid_or_expired_verification:
          "This confirmation link is invalid or has expired. Register again or sign in with Google.",
      };
  ...
  const emailId = "auth-email-login";
  const passwordId = "auth-password-login";
