...
import { decimalHoursToHHMM, sumDayHours, sumWeekHours } from "../lib/time";
...
export default function PayrollRunPage() {
  ...
  const finalize = async () => {
    if (!runId || !confirm("Finalize? You will not be able to edit times."))
      return;
    setBusy(true);
    try {
  ...
  return (
    <div className="max-w-6xl mx-auto">
          ...
          {draft && (
            <>
              <button
                ...
                disabled={busy}
                onClick={() => calculate()}
                className="inline-flex items-center gap-2 rounded-lg bg-teal-600 text-white px-4 py-2 text-sm font-medium disabled:opacity-50"
              >
      ...
      {msg && (
        <p className="no-print text-sm text-teal-600 mb-4">{msg}</p>
      )}
